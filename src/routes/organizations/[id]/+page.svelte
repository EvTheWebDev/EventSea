<script>
    ...
    async function toggleFollow() {
        ...
        try {
            if (isFollowing) {
                // Unfollow logic
                await updateDoc(orgRef, { followers: arrayRemove(auth.currentUser.uid) });
                isFollowing = false;
                followerCount--; 
            } else {
                // Follow logic
                await updateDoc(orgRef, { followers: arrayUnion(auth.currentUser.uid) });
                isFollowing = true;
                followerCount++;
            }
        } catch (err) {
